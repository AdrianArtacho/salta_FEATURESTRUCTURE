...
If the file doesn't exist at the original path, it constructs the alternative path by replacing the root folder "OUTPUT" with "INPUT".
Then, it checks if the file exists at the alternative path.
If the file is found at either the original or alternative path, it prints the path where the file was found. Otherwise, it prints "File not found".

"""

import os

def main(original_path, orig_folder="OUTPUT", alter_folder="INPUT", verbose=False):
    # Original file path
